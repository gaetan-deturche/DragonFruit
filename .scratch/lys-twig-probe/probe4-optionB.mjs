/**
 * PROBE v4 — verify Option B: raycast in RAW frame, map normal to world.
 *
 * Strategy (frame-correct):
 *   1. Build mesh in RAW geometry frame (no transform). Authored RAW contact point
 *      sits on it (probe2: 0.000mm).
 *   2. Cast along -authoredRawNormal at the RAW contact point; pick nearest hit;
 *      smoothed normal = TRUE surface normal in RAW frame.
 *   3. Map that raw-frame normal to the importer's WORLD frame via transformObjectNormal
 *      (the SAME mapping convertLysData uses for authored normals: inv-scale → quaternion).
 *   4. Compare to probe2's world-equivalent and to the authored world normal.
 *
 * Expected: HITs at ~0mm float in raw frame; world-mapped surface normal differs from
 * authored world normal by ~111° (base) / ~50° (tip) — the real correction, applied in
 * a frame that actually hits the mesh.
 *
 * Run: node .scratch/lys-twig-probe/probe4-optionB.mjs
 */
...
// RAW-frame mesh (identity transform) — where authored raw points live.
const meshRaw=new THREE.Mesh(geo,new THREE.MeshBasicMaterial({side:THREE.DoubleSide}));
meshRaw.updateMatrixWorld(true);
geo.computeBoundingSphere();
const FAR=geo.boundingSphere.radius*2+50;

// transformObjectNormal — port of convertLysData.ts:142-157 (maps raw normal → world)
function transformObjectNormal(v){const n=new THREE.Vector3(v.x,v.y,v.z);const ix=Math.abs(objScale.x)>1e-8?1/objScale.x:0,iy=Math.abs(objScale.y)>1e-8?1/objScale.y:0,iz=Math.abs(objScale.z)>1e-8?1/objScale.z:0;n.set(n.x*ix,n.y*iy,n.z*iz);n.applyQuaternion(objQuat);if(n.lengthSq()>1e-8)n.normalize();return n;}

// Option-B raycast: raw frame, map normal to world.
function rayNormalRawFrame(rawContact, rawAuthoredNormal){
  const aN=new THREE.Vector3(rawAuthoredNormal.x,rawAuthoredNormal.y,rawAuthoredNormal.z).normalize();
  const dir=aN.clone().multiplyScalar(-1);
  const origin=new THREE.Vector3(rawContact.x,rawContact.y,rawContact.z).addScaledVector(dir,-FAR);
  const rc=new THREE.Raycaster();rc.set(origin,dir);
  const hits=rc.intersectObject(meshRaw,false);
  if(!hits.length)return null;
  const contactV=new THREE.Vector3(rawContact.x,rawContact.y,rawContact.z);
  let best=hits[0],bd=best.point.distanceTo(contactV);
  for(const h of hits){const d=h.point.distanceTo(contactV);if(d<bd){best=h;bd=d;}}
  const smRaw=smoothedNormal(best);
  const worldN=transformObjectNormal(smRaw); // raw surface normal → world frame
  return {floatOff:bd, rawNormal:smRaw, worldNormal:worldN};
}

const angleDeg=(a,b)=>THREE.MathUtils.radToDeg(new THREE.Vector3(a.x,a.y,a.z).angleTo(new THREE.Vector3(b.x,b.y,b.z)));
const s573=scene.supports.present.byId.s573;

console.log('=== PROBE v4: Option B (raw-frame raycast → world normal) ===\n');
for(const which of ['base','tip']){
  const isBase=which==='base';
  const rawC=isBase?s573.base:s573.tip;
  const rawN=isBase?s573.baseNormal:s573.tipNormal;
  const authoredWorld=transformObjectNormal(rawN); // what the importer assigns today
  const res=rayNormalRawFrame(rawC,rawN);
  console.log(`twig s573 disk ${isBase?'A (base)':'B (tip)'}:`);
  console.log(`  authored WORLD normal:  (${authoredWorld.x.toFixed(3)},${authoredWorld.y.toFixed(3)},${authoredWorld.z.toFixed(3)})`);
  if(!res){console.log('  RAW-FRAME RAYCAST MISS → fallback to authored\n');continue;}
  console.log(`  raw-frame raycast: floatOff=${res.floatOff.toFixed(4)}mm`);
  console.log(`  surface WORLD normal:   (${res.worldNormal.x.toFixed(3)},${res.worldNormal.y.toFixed(3)},${res.worldNormal.z.toFixed(3)})`);
  console.log(`  >>> CORRECTION: ∠(authored, surface) = ${angleDeg(authoredWorld,res.worldNormal).toFixed(2)}°`);
  console.log('');
}
console.log('=== expected: floatOff ~0mm; corrections ~111° (base), ~50° (tip) matching probe2 ===');
